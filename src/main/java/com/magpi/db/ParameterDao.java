...
public class ParameterDao {
    public static class Param { public final double head; public final double coil; public Param(double h,double c){head=h;coil=c;} }
    ...
    public Param getCurrent(String partDescription) throws SQLException {
        ...
        try (Connection c = Database.getInstance().getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return new Param(rs.getDouble(1), rs.getDouble(2));
                return null;
            }
        }
    }
    ...
    public void saveCurrent(String partDescription, double head, double coil) throws SQLException {
        ...
        // Append to history
        try (Connection c = Database.getInstance().getConnection();
             PreparedStatement ps = c.prepareStatement("INSERT INTO parameter_history(part_id, headshot, coilshot) VALUES(?,?,?)")) {
            ps.setInt(1, partId);
            ps.setDouble(2, head);
            ps.setDouble(3, coil);
            ps.executeUpdate();
    ...
    public List<Param> getHistory(String partDescription) throws SQLException {
        ...
        try (Connection c = Database.getInstance().getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) list.add(new Param(rs.getDouble(1), rs.getDouble(2)));
                return list;
            }
        }
    }
