...
public class SessionPartDao {
    ...
    public void updateDemagStatus(long partId, String demagStatus) throws SQLException {
        String sql = "UPDATE session_parts SET demag_status = ? WHERE id = ?";
        try (Connection c = Database.getInstance().getConnection();
                PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, demagStatus);
            ps.setLong(2, partId);
            ps.executeUpdate();
        }
    }
    ...
    /**
     * Checks if a given part number/description combination already exists in
     * history.
     * This is used to prevent creating a new base part with a number that has
     * already
